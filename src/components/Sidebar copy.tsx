...
const Sidebar: React.FC<SidebarProps> = ({ setSelectedForm, anchor, open, toggleDrawer }) => {
  ...
  const changeLanguage = (lng: string) => {
    ...
    if (!open) setIsSettingsOpen(false); // بستن زیرمنو در حالت بسته
  };
  ...
  return (
    <Drawer
      ...
      sx={{
        width: drawerWidth,
        flexShrink: 0,
        transition: 'width 0.15s ease', // انیمیشن سریع‌تر
        [`& .MuiDrawer-paper`]: {
          width: drawerWidth,
            ...
            <List component="div" disablePadding sx={{ pl: open ? 4 : 2 }}>
              <ListItem disablePadding>
                <ListItemButton onClick={() => changeLanguage('en')}>
                  <ListItemIcon sx={{ color: theme === 'light' ? '#000000' : '#e2e8f0', minWidth: open ? 40 : 32 }}>
                    <Language />
                  </ListItemIcon>
                  <ListItemText
